...
const AddRoom = () => {
  ...
  const onSubmitHandler = async (e) => {
    ...
    try {
      ...
      //Convert amenities object to array of selected amenities and keeping only enabled amenities
  ...
  return (
    <form onSubmit={onSubmitHandler}>
      ...
      <div className='grid grid-cols-2 sm:flex gap-4 my-2 flex-wrap'>
        {Object.keys(images).map((key)=>(
          <label htmlFor={`roomImage${key}`} key={key}>
            ...
            {/* onchange update image state varaible */}
          </label>
        ))}
      </div>
      ...
      <div className='flex flex-col flex-wrap mt-1 text-gray-400 max-w-sm'>
          {Object.keys(inputs.amenities).map((amenity,index)=>(
            <div key={index}>
               ...
               // {/* onchange make true to false or vice-versa */}
      ...
      <button className='bg-primary text-white px-8 py-2 rounded mt-2 cursor-pointer' disabled={loading}>
